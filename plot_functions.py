...
def plot_PTRMS_decay(df, parent_compound, fragments, labels, t_zero, ts_UV_off, ts_exp_end, RH):
    ...
    UV_on_df = running_mean(df, [parent_compound], 'Time', '1min', 
                            [pd.to_datetime(t_zero) + pd.Timedelta(minutes = 10), pd.to_datetime(ts_UV_off) - pd.Timedelta(minutes = 10)])
    ...
    print(f'{t_zero.split(' ')[0]}, {RH}, UV on: ln[{labels[0]}] = ({on_values[0]}+-{on_errors[0]})t + ({on_values[1]}+-{on_errors[1]})')
    ...
    print(f'{t_zero.split(' ')[0]}, {RH}, UV off: ln[{labels[0]}] = ({off_values[0]}+-{off_errors[0]})t + ({off_values[1]}+-{off_errors[1]})')
...
def plot_AURA_overview(daq, smps, ams, timestamps, bg_timestamps, t_zero, RH, save_path):
    ...
    if ams is not None:
        fig, ax = plt.subplots(4, 1, figsize = (6.3, 10.5), sharex = True)
        ...
        plot_total(ax[3], ams, 'Ratio_O_C', 'tab:cyan', t_zero)
        ax[3].tick_params(axis = 'y', labelcolor = 'tab:cyan')
        ax[3].set_ylabel('O:C', color = 'tab:cyan')
    ...
    ax[2].set_ylabel('Concentration (# cm$^{-3}$)', color = 'purple')
    if ams is not None:
        ax[2].set_xlabel(None)
    ax2_2 = ax[2].twinx()
    plot_total(ax2_2, smps, 'Geo. Mean (nm)', 'green', t_zero)
    ax2_2.tick_params(axis = 'y', labelcolor = 'green')
    ax2_2.set_ylabel('Geo. mean D$_{p}$ (nm)', color = 'green')
...
def plot_AMS(df, PToF_df, t_zero, timestamps, bg_timestamps, runlength, RH, save_path):
    ...
    for color, key in zip(['#00cc00', '#0000ff', '#ff0000', '#ffaa00', '#ff00cc'], species_keys):
    ...
    for color, key in zip(['#00cc00', '#7f0073', '#ff00e6', '#009900'], family_keys[:4]):
    ...
    for color, key in zip(['#9039e6', '#205f7f'], family_keys[4:]):
...
def plot_SASS(df, timestamps, run_length, datatype, name, save_path):
    ...
    for i, dtype in enumerate(datatype):
        ...
        fig1.tight_layout()
        fig1.savefig(f'{save_path}Timeseries_{name}_{dtype}.jpg', dpi = 600)

        fig2, ax2 = plt.subplots(figsize = (4,3))
        SASS_running_SizeDist(fig2, ax2, new_df, run_length, ['Particle diameter (nm)', ax_labels[i]])
        fig2.tight_layout()
        fig2.savefig(f'{save_path}SizeDist_{name}_{dtype}.jpg', dpi = 600)
